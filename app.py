    ...
    if send and user_question.strip():
        if not st.session_state.pipeline_ready:
            ...
            with st.spinner("Peter Lynch is thinking..."):
                answer = ask_lynch(question=user_question,
...
with tab_analyzer:
    ...
    def fetch_fundamentals(ticker):
        try:
            ...
            eps_growth = all_data.get(
                "earningsGrowth") or all_data.get("revenueGrowth")
            ...
            price = all_data.get("currentPrice") or all_data.get(
                "regularMarketPrice")
            ...
            dividend = summary.get("dividendYield") or summary.get(
                "trailingAnnualDividendYield")

            def classify():
                if eps_growth is None:
                    return "Unknown", "#A0A0A0"
                if eps_growth < 5:
                    return "Slow Grower", "#A0A0A0"
                if eps_growth < 15:
                    return "Stalwart", "#4A90D9"
                if eps_growth >= 20:
                    return "Fast Grower", "#4CAF50"
    ...
    def lynch_backtest(hist):
        ...
        df["position"] = df["signal"].replace(
            0, np.nan).ffill().fillna(0).clip(lower=0)
    ...
    if analyze_btn and ticker_input.strip():
        ...
        for ticker in tickers:
            ...
            with score_col:
                ...
                if scores >= 5:
                    verdict, vc = "Strong Lynch Pick",         "#4CAF50"
                elif scores >= 3:
                    verdict, vc = "Decent Candidate",          "#FFC107"
                else:
                    verdict, vc = "Doesn't Pass Lynch's Test", "#C8102E"
                st.markdown(
                ...
                with chart_col:
                    ...
                    fig.update_layout(
                        ...
                        legend=dict(bgcolor="#2A2A2A",
                                    bordercolor="#3D3D3D", font=dict(size=11)),
                ...
                with bt_col:
                    if not bt.empty:
                        ...
                        fig2.update_layout(
                            ...
                            font=dict(color="#E8E8E8", family="Lato"),
                            legend=dict(
                                bgcolor="#2A2A2A", bordercolor="#3D3D3D", font=dict(size=11)),
...
with tab_dashboard:
    ...
    with ov_c3:
        rm = st.selectbox(
            "Remove", ["--"] + st.session_state.overview_tickers, key="ov_remove")
        if rm != "--":
...
with tab_kmeans:
    ...
    def fetch_fin_data_yahooquery(tickers):
        ...
        if static_used > 0:
            st.info(
                f"Using cached data for {static_used} stock(s). Live data for {live_count} stock(s).")
    ...
    def run_professor_kmeans(fin_data_df):
        ...
        for cid in range(4):
            ...
            avg_roe = cd["returnOnEquity"].mean(
            ) if "returnOnEquity" in cd.columns else 0
            avg_eg = cd["earningsGrowth"].mean(
            ) if "earningsGrowth" in cd.columns else 0
            avg_debt = cd["debtToEquity"].mean(
            ) if "debtToEquity" in cd.columns else 0
            avg_pm = cd["profitMargins"].mean(
            ) if "profitMargins" in cd.columns else 0
    ...
    if run_km:
        tickers_km = tuple(t.strip().upper()
                           for t in ticker_input_km.split(",") if t.strip())
        if len(tickers_km) < 4:
            ...
            if X_scaled is None:
                ...
                st.success(
                    f"K-Means complete -- {len(tickers_km)} stocks in 4 clusters")
                ...
                for cid in range(4):
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        legendgroup="Neutral" if is_neutral else get_label(
                            cid),
                ...
                fig.update_layout(
                    ...
                    font=dict(color="#E8E8E8", family="Lato"),
                    legend=dict(bgcolor="#2A2A2A",
                                bordercolor="#3D3D3D", font=dict(size=12)),
                    xaxis=dict(title="Value", gridcolor="#3D3D3D",
                               zeroline=False),
                    yaxis=dict(title="Quality",
                               gridcolor="#3D3D3D", zeroline=False),
                ...
                with long_col:
                    st.markdown(
                        "<h3 style='color:#4CAF50;'>Long Stocks</h3>", unsafe_allow_html=True)
                    st.markdown(
                ...
                with short_col:
                    st.markdown(
                        "<h3 style='color:#C8102E;'>Short Stocks</h3>", unsafe_allow_html=True)
                    st.markdown(
                        f"<p style='color:#A0A0A0;font-size:0.8rem;margin-top:-8px;'>"
